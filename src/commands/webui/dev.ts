...
const messages = Messages.loadMessages('@salesforce/plugin-webui-dev', 'webui.dev');

export default class WebUIDev extends SfCommand<WebAppDevResult> {
   ...
   */
  private static async promptWebUiSelection(webapps: DiscoveredWebapp[]): Promise<DiscoveredWebapp> {
    const WARNING = '\u26A0\uFE0F'; // ⚠️
  ...
  private static async pollUntilReachable(
    ...
    if (await WebUIDev.isUrlReachable(url)) {
      return true;
    }
    ...
    return WebUIDev.pollUntilReachable(url, timeoutMs, intervalMs, start);
  }
  ...
  public async run(): Promise<WebAppDevResult> {
    ...
    try {
      ...
      if (!discoveredWebapp) {
        ...
        selectedWebapp = await WebUIDev.promptWebUiSelection(allWebapps);
      } else {
      ...
      const isReachable = await WebUIDev.isUrlReachable(resolvedUrl);
      if (isReachable) {
        ...
        const pollPromise = WebUIDev.pollUntilReachable(resolvedUrl, 60_000);
      ...
      const viteProxyActive = await WebUIDev.checkViteProxyActive(devServerUrl);
      ...
      if (flags.open) {
        ...
        await WebUIDev.openBrowser(finalUrl);
      }
      ...
      throw new SfError(`❌ Failed to start webui dev command: ${errorMessage}`, 'UnexpectedError', [
