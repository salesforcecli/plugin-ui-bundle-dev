...
/**
 * Spawn `sf multi dev` asynchronously and wait for the JSON URL line on stderr.
 *
...
export function spawnWebappDev(args: string[], options: { cwd: string; timeout?: number }): Promise<WebappDevHandle> {
  ...
  const proc = spawn(
    process.execPath,
    ['--loader', 'ts-node/esm', '--no-warnings=ExperimentalWarning', binDev, 'multi', 'dev', ...args],
    {
      cwd: options.cwd,
  ...
  return new Promise<WebappDevHandle>((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timeoutId);
      if (code !== null && code !== 0) {
        reject(new Error(`multi dev exited with code ${code}.\nstderr:\n${stderrData}`));
      }
    });
  });
}
