 ...
 * ------------------------------------------------------------------ */
describe('webui dev NUTs — Tier 1 (no auth)', () => {
  ...
  it('should require --target-org', () => {
    const result = execCmd('webui dev --json', {
      ensureExitCode: 1,
      cwd: session.dir,
    });
 ...
 * ------------------------------------------------------------------ */
describe('webui dev NUTs — Tier 2 CLI validation', () => {
  let session: TestSession;
  ...
  it('should error when --name does not match any webapp', () => {
    ...
    const result = execCmd(`webui dev --name NonExistent --target-org ${targetOrg} --json`, {
      ensureExitCode: 1,
      cwd: projectDir,
    });
  ...
  it('should error on --name conflict when inside a different webapp', () => {
    ...
    execSync('sf template generate webui --name appB', {
      cwd: projectDir,
      stdio: 'pipe',
    ...
    const result = execCmd(`webui dev --name appB --target-org ${targetOrg} --json`, {
      ensureExitCode: 1,
      cwd: cwdInsideAppA,
    });
  ...
  // webui/ folder exists but is empty → WebappNotFoundError.
  it('should error when webui folder is empty', () => {
  ...
  // webui/orphanApp/ exists but has no .webapplication-meta.xml → not a valid webapp.
  ...
  it('should use --name to select one webapp when multiple exist', () => {
    ...
    const result = execCmd(`webui dev --name appA --target-org ${targetOrg} --json`, {
      ensureExitCode: 1,
      cwd: projectDir,
    });
  ...
  // When cwd is inside webui/myApp/, discovery auto-selects that
  ...
  // When multiple webapps exist and cwd is inside webui/appA/,
  ...
  it('should error when --url is unreachable', () => {
    ...
    const result = execCmd(`webui dev --name myApp --url http://localhost:5179 --target-org ${targetOrg} --json`, {
      ensureExitCode: 1,
      cwd: projectDir,
    });
  ...
  it('should error when dev.url is unreachable and no dev.command', () => {
    ...
    const result = execCmd(`webui dev --name myApp --target-org ${targetOrg} --json`, {
      ensureExitCode: 1,
      cwd: projectDir,
    });
