...
describe('ErrorHandler', () => {
  ...
  describe('Manifest Errors', () => {
    it('should create manifest not found error', () => {
      ...
      expect(error.message).to.include('not found');
      expect(error.actions?.[0]).to.include('sf webapp generate');
    });
  ...
  describe('Runtime Error Methods', () => {
    it('should create runtime error', () => {
      const error = ErrorHandler.createRuntimeError('Something went wrong', 'file.ts:10:5');

      expect(error).to.be.instanceOf(SfError);
      expect(error.message).to.include('Runtime error occurred');
      expect(error.message).to.include('file.ts:10:5');
      expect(error.message).to.include('Something went wrong');
      expect(error.name).to.equal('RuntimeError');
      expect(error.actions).to.have.lengthOf.at.least(1);
    });

    it('should create uncaught exception error', () => {
      const originalError = new TypeError('Cannot read property of undefined');
      const error = ErrorHandler.createUncaughtExceptionError(originalError);

      expect(error).to.be.instanceOf(SfError);
      expect(error.message).to.include('Uncaught exception');
      expect(error.message).to.include('TypeError');
      expect(error.message).to.include('Cannot read property');
      expect(error.name).to.equal('UncaughtException');
      expect(error.actions).to.have.lengthOf.at.least(1);
    });

    it('should create unhandled rejection error', () => {
      const error = ErrorHandler.createUnhandledRejectionError('Promise rejected');

      expect(error).to.be.instanceOf(SfError);
      expect(error.message).to.include('Unhandled promise rejection');
      expect(error.message).to.include('Promise rejected');
      expect(error.name).to.equal('UnhandledRejection');
      expect(error.actions).to.have.lengthOf.at.least(1);
    });

    it('should create unhandled rejection from Error object', () => {
      const originalError = new Error('Async operation failed');
      const error = ErrorHandler.createUnhandledRejectionError(originalError);

      expect(error).to.be.instanceOf(SfError);
      expect(error.message).to.include('Async operation failed');
    });
  });

  describe('Context-Aware Suggestions', () => {
    it('should provide suggestions for connection refused errors', () => {
      ...
      expect(suggestions.some((s) => s.toLowerCase().includes('server'))).to.be.true;
    });

    it('should provide suggestions for module not found errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('install'))).to.be.true;
    });
    ...
    it('should provide suggestions for syntax errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('syntax'))).to.be.true;
    });

    it('should provide suggestions for type errors', () => {
      const error = new TypeError('Cannot read property of undefined');

      const suggestions = ErrorHandler.getSuggestionsForError(error);

      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('type'))).to.be.true;
    });

    it('should provide suggestions for reference errors', () => {
      const error = new ReferenceError('Variable is not defined');

      const suggestions = ErrorHandler.getSuggestionsForError(error);

      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('variable'))).to.be.true;
    });

    it('should provide suggestions for auth errors', () => {
      const error = new Error('Authentication failed');

      const suggestions = ErrorHandler.getSuggestionsForError(error);

      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('auth'))).to.be.true;
    });
    ...
    it('should provide suggestions for file not found errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('file'))).to.be.true;
    });
    ...
    it('should provide suggestions for permission errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('permission'))).to.be.true;
    });

    it('should provide suggestions for port in use errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.some((s) => s.toLowerCase().includes('port'))).to.be.true;
    });
    ...
    it('should provide generic suggestions for unknown errors', () => {
      ...
      expect(suggestions).to.be.an('array');
      expect(suggestions.length).to.be.greaterThan(0);
      expect(suggestions.some((s) => s.toLowerCase().includes('error message'))).to.be.true;
    });
  });
});
